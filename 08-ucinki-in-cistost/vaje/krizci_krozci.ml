...
[*----------------------------------------------------------------------------*)
type grid_line = player option * player option * player option


type grid = grid_line * grid_line * grid_line

(*----------------------------------------------------------------------------*]
 Definirajte vrednost, ki predstavlja prazno mrežo.  
[*----------------------------------------------------------------------------*)
let empty_grid_line = (None, None, None)

let empty_grid : grid = (empty_grid_line, empty_grid_line, empty_grid_line)
...
let is_full_row (x0, x1, x2) = List.for_all (fun x -> x <> None) [x0; x1; x2]
...
let winner_of_list triples : player option = 
  let status = List.map winner_of_triple triples in
  find_winner status
...
let place_token player grid (row_i, col_i) : state =
  ...
  | Some player ->
      ...
      zmagovalcem *)
      GameOver {result = Win player; final_grid = updated_grid}
